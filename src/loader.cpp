...
bool Loader::loadPointcloudFromROSBag(const std::string &bag_path,
    ...
    for (const rosbag::MessageInstance &m: view) {
        std::cout << " Loading scan: \033[1m" << scan_num++ << "\033[0m from ros bag"
                  << '\r' << std::flush;
...
bool Loader::loadTformFromROSBag(const std::string &bag_path, Odom *odom) {
    ...
    for (const rosbag::MessageInstance &m: view) {
        std::cout << "Loading imu: \033[1m" << imu_num++ << "\033[0m from ros bag" << '\r' << std::flush;
...
/*
  ...
  size_t tform_num = 0;
  for (const rosbag::MessageInstance& m : view) {
    std::cout << " Loading transform: \033[1m" << tform_num++
              << "\033[0m from ros bag" << '\r' << std::flush;
...
bool Loader::loadTformFromMaplabCSV(const std::string &csv_path, Odom *odom) {
    ...
    while (file.peek() != EOF) {
        std::cout << " Loading transform: \033[1m" << tform_num++
                  << "\033[0m from csv file" << '\r' << std::flush;
