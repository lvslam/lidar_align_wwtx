...
const Timestamp &OdomTformData::getTimestamp() const {
...
const Pointcloud &Scan::getRawPointcloud() const {
    return raw_points_;
}
...
const size_t Lidar::getNumberOfScans() const {
    return scans_.size();
}
...
const LidarId &Lidar::getId() const {
    return lidar_id_;
}
...
const Transform &Lidar::getOdomLidarTransform() const {
    return T_o_l_;
}
